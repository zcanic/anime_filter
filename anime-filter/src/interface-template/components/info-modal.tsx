...
interface Anime {
  ...
  episodes: number
  year: number
  tags: string // Changed to string to match App.tsx
}
...
export function InfoModal({ anime, onClose, onMarkInterested, isInterested }: InfoModalProps) {
  ...
  // Ensure tags is a string before splitting. If it's already an array (runtime fallback), join it or handle gracefully.
  // Based on current architecture, it enters as a string.
  const tagList = anime.tags 
    ? (typeof anime.tags === 'string' ? anime.tags.split('|').map(t => t.trim()) : [])
    : [];
